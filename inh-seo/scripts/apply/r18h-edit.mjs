/* Round 18h — the Lugano TIER LINE in dynamic-saunas-review, relabelled to the catalogue's own names:
 *   DYN-6336-02        $2,699  "Low EMF FAR Infrared"
 *   DYN-6336-02 Elite  $3,499  "Ultra Low EMF"
 *   DYN-6336-03 FS     $3,899  "Full Spectrum Near Zero EMF"   (already correct)
 * HELD: the restatement "also offered as a FAR-infrared version ($2,699)…" — outside the authorised line.
 *   node scripts/apply/r18h-edit.mjs [--apply]
 */
...
const SPEC = {
  'dynamic-saunas-review': [
    { kind: 'replace', note: 'Lugano tier line — labels to the catalogue\'s own naming',
      from: 'from $2,699 (FAR) to $3,499 (Low EMF) to $3,899 (Near Zero full spectrum)',
      to:   'from $2,699 (Low EMF) to $3,499 (Ultra Low EMF) to $3,899 (Near Zero full spectrum)' },
  ],
};
const GONE = { 'dynamic-saunas-review': ['$2,699 (FAR)', '$3,499 (Low EMF)'] };
...
for (const [handle, edits] of Object.entries(SPEC)) {
  ...
  if (process.argv.includes('--inject-link')) { if (APPLY) process.exit(1); const b1 = body; body = body.replace('</p>', '<a href="https://example.com/x">x</a></p>'); if (body === b1) { console.log('refusing: injection did not land'); process.exit(1); } }
...
const bpath = backup('r18h-edit', snap);
for (const p of plan) {
  ...
  logChange({ resource: p.a.id, handle: p.a.handle, field: 'body', old: 'Round 18h before-state', new: 'Round 18h edits', note: `backup ${bpath}` });
